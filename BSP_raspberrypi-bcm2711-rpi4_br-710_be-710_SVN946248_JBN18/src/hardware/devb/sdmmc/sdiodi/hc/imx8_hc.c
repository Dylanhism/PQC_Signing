...
static int imx_sdhcx_intr_event(sdio_hc_t *hc)
{
    ...
    if ((cmd->opcode == SD_SEND_TUNING_BLOCK) || (cmd->opcode == MMC_SEND_TUNING_BLOCK)) {
        if ((bs_cfg->tuning == IMX_USDHC_TUNING_MANUAL) && (sts & IMX_USDHC_INT_STATUS_BRR_MASK)) {
 ...
 * @return EOK always.
 */
static int imx_sdhcx_abort(__attribute__((unused)) sdio_hc_t *hc, __attribute__((unused)) sdio_cmd_t *cmd)
{
    return (EOK);
}
...
int imx_sdhcx_host_sdll(sdio_hc_t *hc, unsigned val)
{
    ...
    if (((hc->timing == TIMING_HS400) || (hc->timing == TIMING_HS400ES))) {
        ...
        sdio_slogf(_SLOGC_SDIODI, _SLOG_INFO, hc->cfg.verbosity, 1, "SDLL status 0x%x \n", in32(base + IMX_USDHC_STROBE_DLL_STATUS));
    } else {
        return ENOTSUP;
    }
...
static int imx_sdhcx_clk(sdio_hc_t *hc, int clk)
{
    ...
    while ((hc->clk_max / (pre_div * div) > (uint32_t)clk) && (div < 16)) {
        div++;
    }
    ...
    if (((hc->timing == TIMING_HS400) || (hc->timing == TIMING_HS400ES)) && (clk > 100000000)) {
        out32(base + IMX_USDHC_STROBE_DLL_CTRL, (IMX_USDHC_STROBE_DLL_CTRL_RESET_MASK |
                                                 IMX_USDHC_STROBE_DLL_CTRL_ENABLE_MASK |
                                                 (IMX_USDHC_STROBE_DLL_CTRL_SLV_UPDATE_INT_DEFAULT_VALUE << IMX_USDHC_STROBE_DLL_CTRL_SLV_UPDATE_INT_SHIFT) |
                                                 ((bs_cfg->sdll << IMX_USDHC_STROBE_DLL_CTRL_SLV_DLY_TARGET_SHIFT) &
                                                  IMX_USDHC_STROBE_DLL_CTRL_SLV_DLY_TARGET_MASK)));
    }
    ...
    if ((hc->timing == TIMING_HS200) && (clk > 100000000)) {
        ...
        sdio_slogf(_SLOGC_SDIODI, _SLOG_INFO, hc->cfg.verbosity, 5, "DLL status: 0x%x \n", in32(base + IMX_USDHC_DLL_STATUS));
    }
    if (((hc->timing == TIMING_HS400) || (hc->timing == TIMING_HS400ES)) && (clk > 100000000)) {
        strdll_ctrl_reg = in32(base + IMX_USDHC_STROBE_DLL_CTRL);
        strdll_ctrl_reg &= ~IMX_USDHC_STROBE_DLL_CTRL_RESET_MASK;
        ...
        sdio_slogf(_SLOGC_SDIODI, _SLOG_INFO, hc->cfg.verbosity, 5, "SDLL status 0x%x \n", in32(base + IMX_USDHC_STROBE_DLL_STATUS));
    }
...
static void imx_sdhcx_pre_tuning(sdio_hc_t *hc, int tuning)
{
    ...
    sdio_slogf(_SLOGC_SDIODI, _SLOG_INFO, hc->cfg.verbosity, 2, "%s: tuning at %d\n", __FUNCTION__, tuning);
 ...
 * @return Execution status.
 */
static int imx_sdhcx_std_tune(sdio_hc_t *hc, int op)
{
    imx_sdhcx_hc_t     *sdhc;
    uint32_t           *buf;
    struct sdio_cmd    *cmd;
 ...
 * @return Execution status.
 */
static int imx_sdhcx_man_tune(sdio_hc_t *hc, int op)
{
    imx_sdhcx_hc_t      *sdhc;
    uintptr_t           base;
    uint32_t            mix_ctl;
...
static int imx_sdhcx_tune(sdio_hc_t *hc, int op)
{
    ...
    if (bs_cfg->tuning == IMX_USDHC_TUNING_STANDARD) {
...
int imx_sdhcx_init(sdio_hc_t *hc)
{
    ...
    if (hwi_off != HWI_NULL_OFF) {
        tag_hwversion = hwi_tag_find(hwi_off, HWI_TAG_NAME_hwversion, 0);
    ...
    if ((cap & IMX_USDHC_HOST_CTRL_CAP_DMAS_MASK)) {
        if (hc->version >= IMX_USDHC_HOST_CTRL_VER_SVN_BV_VER_3) {
            ...
            if ((sdhc->adma = sdio_alloc(sizeof(imx_usdhc_adma32_t) * ADMA_DESC_MAX)) == NULL) {
    ...
    if (bs_cfg->tuning == IMX_USDHC_TUNING_STANDARD) {
        ...
        tuning_ctrl |= IMX_USDHC_TUNING_CTRL_TUNING_STEP(bs_cfg->std_step);
        tuning_ctrl |= IMX_USDHC_TUNING_CTRL_TUNING_START_TAP(bs_cfg->std_start_tap);
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/hardware/devb/sdmmc/sdiodi/hc/imx8_hc.c $ $Rev: 936554 $")
#endif
