...
static int imx6_sdhcx_abort( __attribute__((unused)) sdio_hc_t *hc, __attribute__((unused)) sdio_cmd_t *cmd )
{
	return( EOK );
}
...
static int imx6_sdhcx_signal_voltage( sdio_hc_t *hc, int signal_voltage )
{
	...
	if(( hc->version < IMX6_SDHCX_SPEC_VER_3 ) || ( hc->signal_voltage == (uint32_t)signal_voltage )) {
		return( EOK );
	}
...
int imx6_sdhcx_dinit( sdio_hc_t *hc )
{
	...
	if( sdhc->adma )
		sdio_free( sdhc->adma, sizeof( imx6_sdhcx_adma32_t ) * ADMA_DESC_MAX );
...
int imx6_sdhcx_init( sdio_hc_t *hc )
{
    ...
    if( ( cap & IMX6_SDHCX_CAP_DMA ) ) {
        if( hc->version >= IMX6_SDHCX_SPEC_VER_3 ) {
			...
			if( ( sdhc->adma = sdio_alloc( sizeof( imx6_sdhcx_adma32_t ) * ADMA_DESC_MAX ) ) == NULL) {
				sdio_slogf( _SLOGC_SDIODI, _SLOG_ERROR, hc->cfg.verbosity, 1,
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/hardware/devb/sdmmc/sdiodi/hc/imx6.c $ $Rev: 891444 $")
#endif
