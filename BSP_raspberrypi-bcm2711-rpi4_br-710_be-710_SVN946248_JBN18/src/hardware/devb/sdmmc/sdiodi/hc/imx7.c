...
static int imx_sdhcx_signal_voltage(sdio_hc_t *hc, int signal_voltage)
{
    ...
    if ((hc->version < IMX7_SDHCX_SPEC_VER_3) || (hc->signal_voltage == (uint32_t)signal_voltage)) {
        return (EOK);
    }
...
int imx7_sdhcx_dinit(sdio_hc_t *hc)
{
    ...
    if (sdhc->adma) {
        sdio_free(sdhc->adma, sizeof(imx_sdhcx_adma32_t) * ADMA_DESC_MAX);
    }
...
int imx7_sdhcx_init(sdio_hc_t *hc)
{
    ...
    if ((cap & IMX7_SDHCX_CAP_DMA)) {
        if (hc->version >= IMX7_SDHCX_SPEC_VER_3) {
			...
			if( ( sdhc->adma = sdio_alloc( sizeof( imx_sdhcx_adma32_t ) * ADMA_DESC_MAX ) ) == NULL) {
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/hardware/devb/sdmmc/sdiodi/hc/imx7.c $ $Rev: 891444 $")
#endif
