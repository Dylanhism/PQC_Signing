...
int mmc_rpmb_rw( sdio_dev_t *dev, void *pf, int nf, int flgs )
{
	...
	if( flgs == SCF_DIR_IN ) {
		...
		flags = SCF_CTYPE_ADTC | SCF_DIR_IN;
	}
	else {
		if( ( flgs & SCF_SBC_RLW ) ) {
			sbcflg = SBC_RL_WRITE;
		...
		flags = SCF_CTYPE_ADTC | SCF_DIR_OUT | SCF_RSP_BUSY;
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/hardware/devb/sdmmc/sdiodi/mmc.c $ $Rev: 933186 $")
#endif
