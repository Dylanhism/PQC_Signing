...
_spi_iomsg_write(resmgr_context_t *ctp, io_msg_t *msg, spi_ocb_t *ocb)
{
	...
	if ((msglen > ctp->msg_max_size) || (dev->typed_mem)) {
		...
		status = resmgr_msgread(ctp, dev->buf, nbytes, sizeof(spi_msg_t));
		if (status < 0)
			return errno;
		if (status < nbytes)
			return EFAULT;
	...
	buf = dev->funcs->xfer(drvhdl, _SPI_DEV_WRITE(chip), buf, &nbytes);
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/hardware/spi/master/_spi_iomsg_write.c $ $Rev: 872319 $")
#endif
