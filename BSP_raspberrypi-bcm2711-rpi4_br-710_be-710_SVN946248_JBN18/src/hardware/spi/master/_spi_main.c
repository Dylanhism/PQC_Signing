...
-U uid[:gid] Change to the specified user ID (uid). The uid can be a username
             from the user database or a numeric user ID. With a numeric uid,
             a numeric group ID (gid) can optionally be specified. If a
             numeric gid is not given with a numeric uid, a default group of
             0 (root) is used.
-d           driver module name
-P           device file permissions (default: 0666)
-t           typed_mem to use for message buffers
-v level     change logging verbosity level. (range: 0-7, default: 5)
#endif
...
int main(int argc, char *argv[])
{
 ...
 /* default permission for /dev/spi* entry */
	...
	while ((c = getopt(argc, argv, "u:U:P:v:d:t:")) != -1) {
		switch (c) {
			case 'u':
			...
			case 'P':
				...
				break;
			case 't':
				if(strncmp(optarg,"memory=",7) == 0) {
					typed_mem = strdup(optarg+7);
				} else {
					typed_mem = strdup(optarg);
				}
				break;
			case 'v':
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/hardware/spi/master/_spi_main.c $ $Rev: 887126 $")
#endif
